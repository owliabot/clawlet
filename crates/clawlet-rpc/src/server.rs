...
use crate::handlers;
use crate::types::{
    BalanceQuery, ExecuteRequest, HandlerError, SendRawRequest, SignHashRequest,
    SignMessageRequest, TokenSpec, TransferRequest,
};
...
pub trait ClawletApi {
    ...
    #[method(name = "sign_hash")]
    async fn sign_hash(&self, params: SignHashRequest) -> Result<Value, ErrorObjectOwned>;
...
impl ClawletApiServer for RpcServerImpl {
    ...
    async fn sign_hash(&self, params: SignHashRequest) -> Result<Value, ErrorObjectOwned> {
        ...
        match handlers::handle_sign_hash(&self.state, params) {
            Ok(result) => serde_json::to_value(result).map_err(|e| {
                ErrorObjectOwned::owned(
                    error_code::INTERNAL_ERROR,
                    format!("serialization error: {e}"),
